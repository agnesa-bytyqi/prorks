...
add_action( 'setup_theme', 'wpestate_create_membership_type' );
...
function membership_package( $post ) {
	    ...
	    global $post;
                ...
                $visible_pack_array     =   array( esc_html__('User','wpresidence-core') ,esc_html__('Agent','wpresidence-core'),esc_html__('Agency','wpresidence-core'),esc_html__('Developer','wpresidence-core'));
                ...
                <p class="meta-options third-meta-options">
                    <label for="biling_period">'.esc_html__('Billing Time Unit :','wpresidence-core').'</label><br />
                    '.$billing_select.'
                </p>
                
                <p class="meta-options">
                    <label for="billing_freq">'.esc_html__('Bill every x units','wpresidence-core').' </label><br />
                ...
                <p class="meta-options">
                    ...
                    <label for="mem_list_unl">'.esc_html__('Unlimited listings ?','wpresidence-core').'</label>
                </p>
                ...
                <p class="meta-options">
                    <label for="pack_visible">'.esc_html__('Is visible? ','wpresidence-core').'</label><br />
                    '.$visible_select.'
		</p>
...
function wpestate_check_if_developer_or_agency($user_id){
        $user_agent_id      =   get_user_meta($user_id,'user_agent_id',true);
...
function wpestate_check_for_agency($user_id){
    ...
    if($agent_id!=0){
        ...
        if($post->post_author!=1 && $post->post_author!=0 && !user_can( $post->post_author, 'manage_options' )){
