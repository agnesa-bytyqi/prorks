        ...
        function wpestate_show_stripe_form($ammount,$metadata){
            ...
            if($metadata['pay_type']==1){
                ...
                if( isset( $metadata['is_upgrade']) && $metadata['is_upgrade']==1){
                    print '<span class="wpestate_stripe_booking_prop" data-modalid="modal-'.$rand.'">'.esc_html__('Set as Featured','wpresidence-core').'</span>';
                }else{
                    print '<span class="wpestate_stripe_booking_prop" data-modalid="modal-'.$rand.'">'.esc_html__('Pay with Stripe','wpresidence-core').'</span>';
                }

            }else  if($metadata['pay_type']==3){
                print '<span id="wpestate_stripe_booking_recurring" data-modalid="modal-'.$rand.'" >'.esc_html__('Subscribe with Stripe','wpresidence-core').'</span>';
            }
