...
if( !function_exists('wpestate_ajax_agent_contact_form') ):
    ...
    function wpestate_ajax_agent_contact_form(){
        ...
        echo json_encode(array('sent'=>true,'def_REcords'=>$def_arguments ,'response'=>$response) ); 
...
if( !function_exists('wpestate_ajax_agent_contact_form') ):
    ...
    function wpestate_ajax_agent_contact_form(){
        ...
        if($propid!=0){
            ...
            if($agent_agency_dev_id==0 && $receiver_email=='' && get_post_type($agent_id)=='estate_agency'){
                $receiver_email =   esc_html( get_post_meta($agent_id, 'agent_email', true) );
            }
            
            if($agent_agency_dev_id==0 && $receiver_email=='' && get_post_type($agent_id)=='estate_developer'){
        ...
        if( $duplicate_email_adr!='' ){
            $message = $message.' '.esc_html__('Message was also sent to ','wpresidence-core').$duplicate_email_adr;
            wp_mail($duplicate_email_adr, $subject, $message, $headers);
        }
