...
if(!function_exists('wpestate_custom_fonts_elements')):
    function wpestate_custom_fonts_elements(){
        ...
        if ($h1_fontweight != '') {  
            $style.=  'h1,h1 a,.entry-title, .entry-prop{font-weight:' . $h1_fontweight .';}';
        }
...
function wpestate_share_unit_desing($prop_id,$is_single=0){
    ...
    if(intval($is_single)==1){
        ...
        $twiter_label       =    esc_html__('Twitter','wpresidence');
    ...
    <div class="share_unit">
        <a href="<?php print esc_html($protocol);?>://www.facebook.com/sharer.php?u=<?php echo esc_url($link); ?>&amp;t=<?php echo urlencode(get_the_title()); ?>" target="_blank" class="social_facebook"><?php echo esc_html($facebook_label);?></a>
        <a href="<?php print esc_html($protocol);?>://twitter.com/intent/tweet?text=<?php echo esc_html($twiter_status); ?>" class="social_tweet" target="_blank"><?php echo esc_html($twiter_label);?></a>
        <a href="<?php print esc_html($protocol);?>://pinterest.com/pin/create/button/?url=<?php echo esc_url($link); ?>&amp;media=<?php if (isset( $pinterest[0])){ echo esc_url($pinterest[0]); }?>&amp;description=<?php echo urlencode(get_the_title()); ?>" target="_blank" class="social_pinterest"><?php echo esc_html($pinterest_label);?></a>
        <a href="<?php print esc_html($protocol);?>://api.whatsapp.com/send?text=<?php echo urlencode( get_the_title().' '. esc_url( $link )); ?>" class="social_whatsup" target="_blank"><?php echo esc_html($whatsup_label);?></a>   
