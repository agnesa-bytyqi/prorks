...
if(is_singular('estate_agent')){
    ...
    $whatsup_mess       =   'https://api.whatsapp.com/send?phone='.esc_html($realtor_details['realtor_mobile']).'&text='.esc_html('Hello I\'m interested in one of your listings.','wpresidence');
    ?>
...
<?php }else if(is_singular('estate_property')){
     $realtor_details    =   wpestate_return_agent_details($post->ID);
     $whatsup_mess='https://api.whatsapp.com/send?phone='.esc_html($realtor_details['realtor_mobile']).'&text='.esc_html('Hello I\'m interested in ','wpresidence').'['.get_the_title($post->ID).'] '.get_permalink($post->ID);
...
<a class="wpresidence_button wpresidence_button_inverse realtor_whatsapp" href="<?php echo esc_url($whatsup_mess);?>">
    ...
    <?php esc_html_e('WhatsApp','wpresidece');?>
</a> 
